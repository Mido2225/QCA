...
protocol SendingStudentData {
    func studentData(Data: String)
    
}
//what if you want to make a new

class StudentAnswerVC: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate,SendingStudentData {
...
//MARK: - SetUp UI Elements
    ...
    let ocrTextView: UITextView = {
        let ocr = UITextView()
        ocr.font = UIFont.systemFont(ofSize: 20)
    ...
    let descriptionLbl: UILabel = {
        let des = UILabel()
        des.text = "Press next to continue."
    ...
    lazy var sendButton: UIButton = {
        ...
        send.addTarget(self, action: #selector(nextPress), for: .touchUpInside)
        return send
    }()
    
    
    @objc func nextPress() {
        let professorVC = ProfessorAnswerVC()
        professorVC.delegate = self
        professorVC.modalPresentationStyle = UIModalPresentationStyle.fullScreen
        professorVC.holdingData.text = ocrTextView.text
        present(professorVC, animated: true, completion: nil)
    }
